...
### Changed
- **Repository transferred** to the official Dectyr GitHub
  organization: now at `github.com/dectyr/ha-integration` (was
  `github.com/alexandre0thomas/ha-dectyr`).
- All internal documentation links and `manifest.json` URLs
  updated to the new location.
- The previous URL redirects automatically; existing HACS Custom
  Repository entries continue to work but should be updated for
  long-term reliability.

### Notes
- No code or behavior changes.
- Future releases will be published from the new repository.
